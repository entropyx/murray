...
def generate_pdf(
    ...
    pdf.multi_cell(
        0,
        5,
        f"This report provides information about the the results of the analysis of a treatment on the variable '{col_target}' with a duration of {period} days. "
    ...
    pdf.multi_cell(
        0,
        5,
        f"Based on the analysis of the injected data and configured parameters, "
    ...
    pdf.multi_cell(
        0,
        5,
        "The impact graph below shows the causal effects observed in the study. "
    ...
    pdf.multi_cell(
        0,
        5,
        f"The {mmm_option} is a important value to evaluate the performance of the treatment which calculate the {mmm_option} based on the spend and the incremental. "
...
if file is not None:
    ...
    if data is not None:
        ...
        if col_dates == "" or col_locations == "" or col_target == "":
            ...
            start_treatment = st.date_input(
                "Treatment start date",
                min_value=firt_day,
                max_value=last_day,
                value=firt_day,
            )
            end_treatment = st.date_input(
                "Treatment end date",
                min_value=firt_day,
                max_value=last_day,
                value=last_day,
            ...
            start_idx = (
                filtered_data["time"].dt.date == start_treatment.date()
            ).idxmax()
            ...
            if st.button("Evaluate") or st.session_state.evaluation_button_clicked:
                if not st.session_state.evaluation_button_clicked:
                    ...
                    with st.spinner("Running analysis..."):
                        ...
                        lift_total = (
                            results["treatment"][start_position_treatment:].sum()
                            - results["counterfactual"][start_position_treatment:].sum()
                        )
                        st.session_state.lift_total = round(lift_total, 2)
                        st.session_state.holdout_percentage = round(
                            ((total_Y - treatment_Y) / total_Y) * 100, 2
                        )
                        st.session_state.treatment_group = ", ".join(treatment_group)
                        st.session_state.control_group = ", ".join(
                        ...
                        (
                            ...
                            upper_bound_value,
                            prediction_value,
                        ) = plot_impact_evaluation_streamlit(
                            results, filtered_data, length_treatment
                        )
                ...
                df = pd.DataFrame(
                    {
                        ...
                        "Post-treatment": [
                            ...
                            np.abs(
                                np.sum(post_treatment) - np.sum(post_counterfactual)
                            ),
                        ],
                        "Increment": [" ", " ", " "],
                    }
                ...
                st.write("--------------------------------")
                st.write(
                    '<h4 style="text-align: center;"> Graphical representation of the evaluation</h4>',
                    unsafe_allow_html=True,
                )
