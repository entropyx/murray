...
def test_apply_lift(synthetic_series):
    ...
    assert np.all(
        y_lifted[:50] == y[:50]
    ), "Values before the treatment should not change"
    assert np.all(
        y_lifted[70:] == y[70:]
    ), "Values after the treatment should not change"
    assert np.all(
        y_lifted[50:70] == y[50:70] * 1.1
    ), "The lift should be applied in the treatment period"
...
def test_calculate_conformity(synthetic_series):
    ...
    conformity = calculate_conformity(
        y_real, y_control, start_treatment=50, end_treatment=70
    )

    expected_conformity = np.mean(y_real[50:70]) - np.mean(y_control[50:70])
    assert np.isclose(
        conformity, expected_conformity
    ), "The calculated conformity should match the expected value"
...
def test_evaluate_sensitivity():
    ...
    results_by_size = {
        50: {
            "Actual Target Metric (y)": np.random.rand(100) * 100,
    ...
    assert all(
        isinstance(v, dict) for v in sensitivity_results.values()
    ), "Each value in sensitivity_results must be a dictionary"
    assert all(
