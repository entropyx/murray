...
from Murray.main import BetterGroups, SyntheticControl, select_treatments, select_controls, is_streamlit_context
...
def test_is_streamlit_context():
    ...
    assert not is_streamlit_context(), "Should return False when not in Streamlit context"
    
    os.environ['STREAMLIT_SERVER_PORT'] = '8501'
    assert is_streamlit_context(), "Should return True when STREAMLIT_SERVER_PORT is set"
...
def test_better_groups_with_duplicates(similarity_matrix, correlation_matrix, cleaned_dataframe_with_duplicates):
...
def test_better_groups_with_excluded_locations(similarity_matrix, correlation_matrix, test_data):
    ...
    if results is not None:
        for size, result in results.items():
            ...
            assert not any(loc in excluded for loc in treatment_group), \
