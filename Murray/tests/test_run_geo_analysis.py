...
def test_run_geo_analysis_no_excluded_locations(sample_data):
    ...
    assert isinstance(results, dict), "The result must be a dictionary"
    assert len(results["simulation_results"]) > 0, "Should have results with no exclusions"
...
def test_run_geo_analysis_single_delta_period(sample_data):
    ...
    assert isinstance(results, dict), "The result must be a dictionary"
    assert len(results["series_lifts"]) > 0, "Should have series lifts for single values"
...
def test_run_geo_analysis_all_locations_excluded(sample_data):
    ...
    with pytest.raises(ValueError, match="treatment size.*exceeds.*available number of columns"):
