...
def test_synthetic_control_ridge_adjustment(synthetic_control_with_ridge, synthetic_data):
    ...
    predictions, weights = synthetic_control_with_ridge.predict(X, time_index=time_index)
    
    assert isinstance(predictions, np.ndarray)
    assert len(predictions) == len(y)
    assert not np.isnan(predictions).any()

def test_synthetic_control_filter_controls_by_weights(synthetic_control, synthetic_data):
...
def test_synthetic_control_filter_controls_no_weights_above_threshold(synthetic_control, synthetic_data):
...
def test_synthetic_control_filter_controls_mismatched_lengths(synthetic_control, synthetic_data):
