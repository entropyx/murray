...
def plot_impact_streamlit_app(geo_test, period, holdout_percentage,significance_level=0.05):
        ...
        point_difference = treatment_series - counterfactual
        ...
        x_treatment = list(range(start_treatment, len(counterfactual)))
        y_treatment = treatment_series[start_treatment:]
        ...
        att = np.mean(treatment_series[start_treatment:] - counterfactual[start_treatment:])
        incremental = np.sum(treatment_series[start_treatment:] - counterfactual[start_treatment:])
...
def plot_impact_report(geo_test, period, holdout_percentage,length_treatment, significance_level=0.05):
    ...
    lower_bound_value = (np.sum(lower_bound) - np.sum(counterfactual[start_treatment:])) / np.abs(np.sum(counterfactual[start_treatment:])) * 100
    upper_bound_value = (np.sum(upper_bound) - np.sum(counterfactual[start_treatment:])) / np.abs(np.sum(counterfactual[start_treatment:])) * 100
    prediction_value = (np.sum(treatment_series[start_treatment:]) - np.sum(counterfactual[start_treatment:])) / np.abs(np.sum(counterfactual[start_treatment:])) * 100

    att = np.mean(treatment_series[start_treatment:] - counterfactual[start_treatment:])
    att = att / length_treatment
    incremental = np.sum(treatment_series[start_treatment:] - counterfactual[start_treatment:])
    pre_treatment = treatment_series[start_treatment-period:start_treatment]
    pre_counterfactual = counterfactual[start_treatment-period:start_treatment]
    post_treatment = treatment_series[start_treatment:]
    ...
    format_ticks(axes[0], np.concatenate([counterfactual, treatment_series]))
