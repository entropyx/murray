...
def plot_impact_streamlit_app(
    geo_test, period, holdout_percentage, significance_level=0.05
):
    """
    ...
    available_deltas = [
        delta
        for s, delta, period in series_lifts.keys()
        if s == target_size_key and period == period
    ]

    if not available_deltas:
        logger.debug(
            f"No available deltas for holdout {holdout_percentage}% and period {period}."
        )
    ...
    cumulative_effect = [0] * (len(treatment) - period) + np.cumsum(
    ...
    for i in range(1, 4):
        fig.add_vline(
            x=start_treatment, line=dict(color="black", dash="dash"), row=i, col=1
        )
...
def plot_impact_graphs(geo_test, period, treatment_percentage):
    holdout_percentage = 100 - treatment_percentage
    att, incremental, fig, lower_bound_value, upper_bound_value, prediction_value = (
        plot_impact_streamlit_app(geo_test, period, holdout_percentage)
    )
    return fig
...
def plot_impact_evaluation_streamlit(
    results_evaluation, df, length_treatment, significance_level=0.05
):
    """
    ...
    return (
        ...
        round(att, 2),
        round(incremental, 2),
        round(lower_bound_value, 2),
        round(upper_bound_value, 2),
        round(prediction_value, 2),
    )
...
def plot_impact_graphs_evaluation(results_evaluation):
    fig, att, incremental, lower_bound_value, upper_bound_value = (
        plot_impact_evaluation(results_evaluation)
    )
    return fig
...
def plot_impact_report(
    ...
    def format_ticks(ax, data):
        ...
        ax.yaxis.set_major_formatter(
            ticker.FuncFormatter(lambda x, p: millify(x, precision=1))
        )
    ...
    axes[0].fill_between(
        range(len(counterfactual) - period, len(counterfactual)),
        lower_bound,
        upper_bound,
        color="gray",
        alpha=0.2,
    ...
    axes[1].plot(
        point_difference,
        label="Point Difference (Causal Effect)",
        color=green,
        linewidth=1,
    )
    axes[1].fill_between(
